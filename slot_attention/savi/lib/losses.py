...
@register_loss
def masked_slots(preds: ArrayTree,
                targets: ArrayTree,
                key: str,
                reduction_type: str = "sum",
                **_) -> float:
  ...
  squared_l1_norm_fn = jax.vmap(functools.partial(
      squared_l1_norm, reduction_type=reduction_type))
  



def squared_l1_norm(preds: Array, targets: Array,
                    reduction_type: str = "sum") -> Array:
  if reduction_type == "sum":
    return jnp.sum(jnp.abs(preds - targets))
