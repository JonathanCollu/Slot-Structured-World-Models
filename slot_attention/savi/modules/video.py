...
class SAVi(nn.Module):
  ...
  temporal_predictor: Callable[[], nn.Module]
  initializer: Callable[[], nn.Module]
  param_state_init: Optional[Callable[[], nn.Module]]
  decode_corrected: bool = True
  decode_predicted: bool = True
  masking_token: Array = None
  ...
  def __call__(self, video: Array, conditioning: Optional[Array] = None,
    ...
    if self.param_state_init is not None and self.masking_token is None:
      self.masking_token = self.param_state_init()(None, batch_size=video.shape[0])
    ...
    # Predict slots of missing timesteps
    # TODO remove and replace some timesteps with learnable embeddings
    # print(states.corrected.shape)
    # exit()
    # temporal_predictor = self.temporal_predictor()
    # masked_pred = temporal_predictor(states.corrected, train=train)  # modify states.corrected to use over temporal dimension for each slot
    # masked_pred = self.predictor()(masked_pred, train=train)
